    ...
    <div class="com-container">
        ...
        <div class="table-wrap">
            ...
            <div class="body" ref="bodyRef">
                ...
                <div class="empty-box" v-if="(hasLoad && (!dataList || !dataList.length)) || !bugFlagList.length">
                    <span>暂无数据</span>
                </div>
            </div>
        </div>
...
import {allBugfixOrFlagDetail} from "@/api/compData";
...
const loadData = () => {
    ...
    hasLoad.value = false
    allBugfixOrFlagDetail<any>(curData.value.id, bugType, 6).then(res => {
        dataList.length = 0
        let list = res.list || []
        if (bugType === 1) {
            list = list.map(item => {
