  ...
  <div class="stage">
    <header>
        ...
        <div class="user-info">
          <span>
            ...
            <template v-if="useCompDataStoreHook().onlineInfo"
              ...
              {{
                useCompDataStoreHook().onlineInfo.judgerMaster
                  ? '裁判长'
                  : '裁判员'
              }}
            </template>
      ...
      <div class="sub-title">
        <span
          >DART侦攻防溯平台：{{
            useCompSettingStoreHook().pageIndex === 1 ? '初赛' : '决赛'
          }}</span
        >
      </div>
      <div>
    ...
    <div class="page-wrap">
      <transition name="left">
        <section
          class="main flex main-page"
          v-show="useCompSettingStoreHook().pageIndex === 1"
          key="manPage"
        >
          <div class="flex-col h-100">
            <div class="flex-col flex-2">
              ...
              <div class="flex-sb">
                ...
                <StatNumber
                  ...
                  icon="5"
                  :cnt-num="
                    useCompDataStoreHook().onlineInfo?.containerCnt ?? 0
                  "
                />
              </div>
            ...
            <div class="flex-col h-0 flex-1-5">
              <h3 class="title1" style="margin: 0.3rem 0 0.2rem">
                FLAG提交情况
              </h3>
              <div class="box flex-1 flex-col h-0">
                <FlagGridList
                  class="flex-1"
                  :titles="['题目', '队伍', '运行得分', '时间']"
          ...
          <div class="flex-col h-100">
            <div
              class="flex-col flex-2 h-0"
              style="position: relative; box-sizing: border-box"
            >
              <h3 class="title2" style="margin: 0 0 0.3rem">总分排行</h3>
              <GridList2
                class="flex-1"
                :data-fn="useCompDataStoreHook().loadTotalRanking"
                modify-field="totalSubmitGroupCnt"
              />
            </div>
            <div class="flex-col h-0 flex-1-5">
              <h3 class="title1" style="margin: 0.3rem 0 0.2rem 0">
                算法运行情况
              </h3>
              <div class="box flex-1 flex-col h-0">
                <RecGridList
          ...
          <div class="flex-col">
            <h3 class="title2" style="margin: 0 0 0.3rem">CTF题排行</h3>
            <GridList2
              class="flex-1"
              :data-fn="useCompDataStoreHook().loadCftRanking"
            />
            <h3 class="title2" style="margin: 0.5rem 0 0.3rem">
              数据分析题排行
            </h3>
            <GridList2
              class="flex-1"
              :data-fn="useCompDataStoreHook().loadDataAnalyzeRank"
            />
          </div>
        </section>
      </transition>
      <transition name="right" v-if="useCompSettingStoreHook().hasPageTwo">
        <section
          class="main flex sub-page"
          v-show="useCompSettingStoreHook().pageIndex === 2"
          key="subPage"
        >
          <div class="flex-col">
            <div class="flex-col flex-2">
              ...
              <div class="flex-sb">
                ...
                <StatNumber
                  ...
                  icon="8"
                  :cnt-num="
                    useCompDataStoreHook().totalFinalSubmitGroupCnt ?? 0
                  "
                />
                <StatNumber
                  label="在线容器数统计"
            ...
            <div class="flex-1-3 flex-col box" style="height: 0">
              <GridList3
                class="flex-1"
                :bug-flag-list="useCompDataStoreHook().allBugfixFlagList"
              />
            </div>
          </div>
          <div class="flex-col h-100">
            ...
            <GridList2
              class="flex-1"
              :data-fn="useCompDataStoreHook().loadFinalTotalRanking"
              modify-field="totalFinalSubmitGroupCnt"
            />
          </div>
          <div class="flex-col">
            <h3 class="title2" style="margin: 0 0 0.3rem">攻击得分排行</h3>
            <GridList2
              class="flex-1"
              :data-fn="useCompDataStoreHook().loadFinalFlagRanking"
            />
            <h3 class="title2" style="margin: 0.5rem 0 0.3rem">防守得分排行</h3>
            <GridList2
              class="flex-1"
              :data-fn="useCompDataStoreHook().loadFinalBugfixRanking"
            />
          </div>
        </section>
      </transition>
...
.left-leave-active {
  animation: fold-left-out 0.5s ease-in-out;
}

.right-enter-active {
  animation: fold-right-in 0.5s ease-in-out;
}

.left-enter-active {
  animation: fold-left-in 0.5s ease-in-out;
}

.right-leave-active {
  animation: fold-right-out 0.5s ease-in-out;
}
</style>
...
header {
  ...
  background: linear-gradient(
    to right,
    rgba(14, 31, 55, 0.9),
    rgba(13, 29, 52, 0.8) 50%,
    rgba(12, 22, 38, 0.8) 100%
  );
  margin-bottom: 0.4rem;
  box-shadow: 0 -0.2rem 0.5rem #091421;
  ...
  .sub-title {
    ...
    text-shadow: 0 0 0.1rem rgba(239, 239, 239, 0.5);
    display: flex;
    align-items: center;
  ...
  .user-info {
    ...
    padding: 0.08rem 0.08rem 0.08rem 0.716rem;
    min-width: 4.6rem;
...
.main-page {
  ...
  padding: 0.06rem 0.35rem 0.8rem 0.35rem;
...
.sub-page {
  justify-content: space-around;
  padding: 0.06rem 0.45rem 0.8rem 0.35rem;
  position: relative;
