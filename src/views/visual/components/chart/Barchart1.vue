...
const renderChart = () => {
    ...
    let xdata = {
        value: ['刘亦菲', '赵丽颖', '刘诗诗', '高圆圆', '古力娜扎']
    };
    let sdata = {
        value: [50, 40, 80, 40, 60]
    }
    ...
    let series_sdata = [];
    let colors = [['#56a7bd', '#67ceec'], ['#ea7cac', '#f2a6c2'], ['#6e78ce', '#8897f8'], ['#f3a681', '#f6c99c'], ['#75be5b', '#aff988']]
            ...
            itemStyle: {
                ...
                color: {
                    ...
                    colorStops: [
                        {
                            offset: 0,
                            color: colors[k][0], // 0% 处的颜色
                        },
                        {
                            offset: 1,
                            color: colors[k][1], // 0% 处的颜色
                        },
                    ],
    ...
    function getPRichs() {
        ...
        xdata.value.forEach((item, i) => {
            richs[`p${i}`] = {
                ...
                verticalAlign: 'top',
                lineHeight: 15,
                backgroundColor: colors[i]?.[0] ?? '#73E3FF'
            }
        })
        return richs
    }
