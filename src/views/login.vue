<template>
  <div class="container">
    <header>
      <span class="title">DART侦攻防溯平台</span>
    </header>
    <section>
      <el-input
        class="item"
      ...
      <el-input
        ...
        show-password
      />
      <el-button class="item" @click="login" :loading="isLogin"
        >登 录</el-button
      >
      <div class="tip">比赛前端（裁判端） 公共测试版本</div>
    </section>
    <footer></footer>
  </div>
</template>
...
export default {
  ...
  methods: {
    login() {
      ...
      this.isLogin = true;
      loginByPwd(this.userName, this.pwd, { showMessage: false })
        .then((res) => {
          if (!res.remoteUser) {
